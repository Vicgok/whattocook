...
import { fetchIngredientCategories, fetchIngredients } from "@/repositories/ingredient.repository";
...
export async function listIngredients(search = ""): Promise<Ingredient[]> {
  ...
  return search ? searchIngredients(search, localIngredients).map(({ ingredient }) => ingredient) : localIngredients;
}
