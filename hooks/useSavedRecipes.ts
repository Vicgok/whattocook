...
import { fetchSavedRecipeIds, saveRecipe, unsaveRecipe } from "@/services/saved-recipes.service";
...
export function useSavedRecipeIds(userId?: string, authReady = false) {
  ...
  useEffect(() => {
    if (!enabled) traceSupabaseSkip("savedRecipes", authReady ? "no_user_id" : "auth_not_ready");
  }, [authReady, enabled]);
  ...
  return {
    ...
    save: useMutation({
      ...
      onMutate: async (recipeId) => {
        ...
        queryClient.setQueryData<string[]>(key, (current = []) => current.includes(recipeId) ? current : [recipeId, ...current]);
        return { previous };
      },
      onError: (_error, _recipeId, context) => queryClient.setQueryData(key, context?.previous),
      onSuccess: (recipeId) => queryClient.setQueryData<string[]>(key, (current = []) =>
    ...
    unsave: useMutation({
      ...
      onMutate: async (recipeId) => {
        await queryClient.cancelQueries({ queryKey: key });
        const previous = queryClient.getQueryData<string[]>(key);
        queryClient.setQueryData<string[]>(key, (current = []) => current.filter((id) => id !== recipeId));
        return { previous };
      },
      onError: (_error, _recipeId, context) => queryClient.setQueryData(key, context?.previous),
      onSuccess: (recipeId) => queryClient.setQueryData<string[]>(key, (current = []) =>
