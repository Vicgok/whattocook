...
export function useRecipes() {
  return useQuery({ queryKey: queryKeys.recipes, queryFn: () => {
    traceCache("recipes", "network", "miss");
    traceQueryExecution(queryKeys.recipes, "useRecipes", isSupabaseConfigured);
    return listRecipes();
  }, staleTime: 12 * 60 * 60 * 1000, gcTime: 7 * 24 * 60 * 60 * 1000, meta: { persist: true } });
}
export function useRecipe(id?: string) {
  ...
  return useQuery({
    ...
    queryFn: () => {
      traceQueryExecution(queryKeys.recipe(id ?? ""), "useRecipe", isSupabaseConfigured);
      return getRecipe(id!);
    },
    enabled: Boolean(id),
