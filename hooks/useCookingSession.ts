...
export function useCookingSession(userId?: string, recipeId?: string, authReady = false) {
  const queryClient = useQueryClient();
  const key = queryKeys.cookingSession(userId ?? "guest", recipeId ?? "");
  const enabled = Boolean(authReady && userId && recipeId);
  useEffect(() => {
    if (!enabled) traceSupabaseSkip("cookingSession", authReady ? "no_user_id" : "auth_not_ready");
  }, [authReady, enabled]);
  const session = useQuery({ queryKey: key, queryFn: () => {
  ...
  }, enabled, staleTime: 60 * 60 * 1000, gcTime: 24 * 60 * 60 * 1000, meta: { persist: true }, refetchOnReconnect: false });
  return {
    ...session,
    start: useMutation({
      ...
      mutationFn: () => session.data
        ? Promise.resolve(session.data)
        : startCookingSession(userId!, recipeId!, { knownNoActiveSession: session.isSuccess }),
      retry: 0,
    ...
    updateStep: useMutation({
      ...
      onMutate: async ({ sessionId, step }) => {
        ...
        queryClient.setQueryData<CookingSession | null>(key, (current) => current && current.id === sessionId ? { ...current, currentStep: step } : current);
        return { previous };
      },
