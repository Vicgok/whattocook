...
export function useIngredients(search = "") {
  ...
  }, staleTime: 24 * 60 * 60 * 1000, gcTime: 7 * 24 * 60 * 60 * 1000, meta: { persist: search.trim().length === 0 }, refetchOnReconnect: false });
}
export function useIngredientCategories() {
  return useQuery({ queryKey: queryKeys.categories, queryFn: () => {
    traceCache("ingredientCategories", "network", "miss");
    traceQueryExecution(queryKeys.categories, "useIngredientCategories", isSupabaseConfigured);
    return listIngredientCategories();
  }, staleTime: 24 * 60 * 60 * 1000, gcTime: 7 * 24 * 60 * 60 * 1000, meta: { persist: true }, refetchOnReconnect: false });
}
