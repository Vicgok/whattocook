...
export function singularizeSafe(value: string): string {
  const words = normalizeIngredientText(value).split(" ");
...
export function ingredientLookup(
  ...
  for (const ingredient of ingredients)
    for (const term of [
      ...
      index.set(normalizeIngredientText(term), ingredient);
  return index;
}
export function resolveIngredient(
  ...
  const index = ingredientLookup(ingredients);
  const key = normalizeIngredientText(query);
  return index.get(key) ?? index.get(singularizeSafe(key));
}
