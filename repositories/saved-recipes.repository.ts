...
export async function fetchSavedRecipeIds(userId: string): Promise<string[]> {
  ...
  const { data, error } = await client.from("saved_recipes").select("recipe_id").eq("user_id", userId).order("created_at", { ascending: false });
  if (error) {
...
export async function saveRecipe(userId: string, recipeId: string): Promise<string> {
  const client = getSupabaseClient();
  if (!client) throw new Error("Supabase is not configured.");
  traceSupabaseRequest("savedRecipes.save", "useSavedRecipeIds.save");
  const { error } = await client.from("saved_recipes").upsert({ user_id: userId, recipe_id: recipeId }, { onConflict: "user_id,recipe_id" });
  if (error) {
...
export async function unsaveRecipe(userId: string, recipeId: string): Promise<string> {
  const client = getSupabaseClient();
  if (!client) throw new Error("Supabase is not configured.");
  traceSupabaseRequest("savedRecipes.delete", "useSavedRecipeIds.unsave");
  const { error } = await client.from("saved_recipes").delete().eq("user_id", userId).eq("recipe_id", recipeId);
  if (error) {
