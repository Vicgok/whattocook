...
type PantryContextValue = { pantry: PantryItem[]; addIngredients: (ingredientIds: string[]) => void; removeIngredient: (ingredientId: string) => void };
const PantryContext = createContext<PantryContextValue | undefined>(undefined);
const createPantryItem = (ingredientId: string): PantryItem => ({ id: ingredientId, userId: "local-user", ingredientId, createdAt: "2026-09-07", updatedAt: "2026-09-07" });
export function PantryProvider({ children }: { children: ReactNode }) {
  const [pantry, setPantry] = useState<PantryItem[]>(() => defaultPantryIngredientIds.map(createPantryItem));
  const addIngredients = (ingredientIds: string[]) => setPantry((old) => {
  ...
  const removeIngredient = (ingredientId: string) => setPantry((old) => old.filter((item) => item.ingredientId !== ingredientId));
